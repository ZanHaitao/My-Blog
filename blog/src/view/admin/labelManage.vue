<template>
    <div class="label-manage">
        ...
        <div class="label-manage-title">
            管理标签
            <span class="label-manage-publish" @click="showTip">新增</span>
        </div>
        <div class="label-manage-content">
            <template v-if="labelData.count">
                <table-content :dataList="labelData.data" :type="'标签'" @handleEdit="handleEdit" @handleDelete="handleDelete" />
                <my-pagination :page="page" :limit="limit" :count="labelData.count" @changePage="changePage" />
            </template>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            async getData() {
                this.labelData = await this.$api.getLabelFindByPage({
                    page: this.page,
                    limit: this.limit
                });
            },
            ...
            async handleDelete(data) {
                const result = await this.$api.deleteLabel(data.id)
                if (result) {
            ...
            async handleEdit(data) {
                const result = await this.$api.updateLabel(data.id, {
                    title: data.title,
                })
                if (result) {
            ...
            async clickPublish() {
                const result = await this.$api.addLabel({
                    title: this.title
                })
